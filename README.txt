...
Key Design Notes:
    ...
    - In order to implement our spelling checker program, we decided to utilize three different fixed arrays to extract 
        values from the provided text files/directories in the arguments.
        ...
        - One array stores dictionary word objects, which are structs that hold the original dictionary word, 
            ...
            - This array has a pre-determined length of 200,000 dictionary words, large enough to hold the ilab
                dictionary and other inputted dictionaries.  
        ...
        - The second array stores the names of the text files collected in the second argument, where our 
            ...
            - This array has a pre-determined length of 100 character arrays, holding at max 100 inputted text files.
        
        - The third array stores word objects, which holds the words read from each instance in the text file array, 
            as well as its respective line and column number in the text file. Our method getwords() loops through the 
            text file array, creating a word array for every text file instance and clearing after iteration is complete. 
            These words are generated by reading through each instance in the txt_files array. 
            The inclusion of line and column number in the struct are helpful for reporting errors.

            - to calculate the length of the words array, we created a helper method called wordCounter() that 
                returns the number of words in a given text file. We then create an array of that size and re-calculate
                the length for every following text file. We also make sure to call clearwords() after each text file 
                is read into the words array and binary searched to prevent overwriting issues. 
    ...
    - We decided to check the spelling of all collected words by binary searching the dictionary array, due to its 
        ...
        - If hyphens are present in a word, we use checkHyphens() and splitHyphens() to split the hyphenated words into
            ...
            performed on every non-null word object in that array. An arraylist here is not essential, as hyphenated words
    ...
    - When reading in inputs, our program gets rid of any leading punctuation (quotations or brackets specifically), 
        as well as ALL trailing punctuation. 
        For example, in an input such as "dog.." , the word dog will stored in the array, getting rid of 
        the quotation marks and trailing punctuation.
        Also if an input is  "Doc-tor",  the word that is read in would be Doc-tor . Punctuation that seperates two valid alpha 
            characters is not removed and is counted. 
...
Library Requirements:
    ...
    - the inputted dictionary must have at most 200,000 words in it
        - the dictionary must also be made up of alphanumeric characters
    - the inputted number of text files must be less than or equal to 100
...
Test Scenarios:
    ...
    - To ensure correctness, we tested the following: 
        ...
        - a word with only be accepted if it's corresponding dictionary word is uppercase IF
            all it's letters are capital OR it's a direct match
        ...
        - words that contain non-trailing punctuation (not including apostophes and hyphens) 
            should always be rejected (specifically on ilab dictionary)
